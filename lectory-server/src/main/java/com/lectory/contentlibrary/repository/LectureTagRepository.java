...
public interface LectureTagRepository
        extends JpaRepository<LectureTag, LectureTagId> {   // ✅ 복합 PK 지정

    /* 1) 챕터 ID 기준 태그 전체 삭제 */
    @Transactional
    @Modifying(clearAutomatically = true)
    @Query("DELETE FROM LectureTag lt WHERE lt.lectureId = :lectureId")   // ✅ 경로 수정
    void deleteAllByLectureId(@Param("lectureId") Long lectureId);

    /* 2) 챕터 ID 기준 태그 조회 */
    List<LectureTag> findByLectureId(Long lectureId);                     // ✅ 경로 수정
}
